...
def get_scores(pred_label_dir, gt_label_dir):
    ...
    precision_mean = np.mean([x for x in all_scores['precision'] if np.isnan(x) == False])
    recall_mean = np.mean([x for x in all_scores['recall'] if np.isnan(x) == False])
    recall_median = np.median([x for x in all_scores['recall'] if np.isnan(x) == False])
    jaccard_mean = np.mean([x for x in all_scores['jaccard'] if np.isnan(x) == False])
    seg_score_mean = np.mean([x for x in all_scores['segmentation'] if np.isnan(x) == False])
    ...
    recall1 = np.median([x for x in indiv_scores['recall'][1] if np.isnan(x) == False])
    recall2 = np.median([x for x in indiv_scores['recall'][2] if np.isnan(x) == False])
    precision1 = np.median([x for x in indiv_scores['precision'][1] if np.isnan(x) == False])
    precision2 = np.median([x for x in indiv_scores['precision'][2] if np.isnan(x) == False])
    ...
    print("Median Recall for label 1:", round(recall1, 3))
    print("Median Recall for label 2:", round(recall2, 3))
    print("Median Precision for label 1:", round(precision1, 3))
    print("Median Precision for label 2:", round(precision2, 3))
    print("Mean Jaccard for all classes:", round(jaccard_mean, 3))
    print("Mean Segmentation Score for all classes:", round(seg_score_mean, 3))
...
def get_aggregated_dice(pred_label_dir, gt_label_dir):
    ...
    print('dice:', dices)
    print('median dice:', np.median(dices))
    print('mean dice:', round(np.mean(dices), 3))
...
if __name__ == "__main__":
    ...
    #gt_label_dir = data_dir + '/nnUNet_raw_data_base/nnUNet_raw_data/Task505_PN/labelsTs'
    #pred_label_dir = data_dir + '/all_results/Task505/test1'
    #gt_label_dir = '/mnt/aertslab/USERS/Zezhong/HN_OUTCOME/DFCI/seg_nii'
